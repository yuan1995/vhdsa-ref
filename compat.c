...
void* vhdsa_aligned_alloc(size_t alignment, size_t size) {
  ...
  const size_t offset = alignment - 1 + sizeof(u8);
  u8* buffer     = malloc(size + offset);
  if (!buffer) {
    return NULL;
  }

  u8* ptr   = (u8*)(((uintptr_t)(buffer) + offset) & ~(alignment - 1));
