...
static inline void hash_update(hash_context* ctx, const u8* data, size_t size) {
  if (ctx->shake256) {
...
static inline void hash_squeeze(hash_context* ctx, u8* buffer, size_t buflen) {
  if (ctx->shake256) {
...
static inline void hash_update_x4(hash_context_x4* ctx, const u8** data, size_t size) {
  if (ctx->shake256) {
...
static inline void hash_update_x4_4(hash_context_x4* ctx, const u8* data0,
                                    const u8* data1, const u8* data2,
                                    const u8* data3, size_t size) {
  if (ctx->shake256) {
...
static inline void hash_update_x4_1(hash_context_x4* ctx, const u8* data, size_t size) {
  if (ctx->shake256) {
...
static inline void hash_squeeze_x4(hash_context_x4* ctx, u8** buffer, size_t buflen) {
  if (ctx->shake256) {
...
static inline void hash_squeeze_x4_4(hash_context_x4* ctx, u8* buffer0, u8* buffer1,
                                     u8* buffer2, u8* buffer3, size_t buflen) {
  if (ctx->shake256) {
...
static inline void hash_update_uint32_le(hash_context* ctx, u32 data) {
  data = htole32(data);
  hash_update(ctx, (const u8*)&data, sizeof(data));
}
...
static inline void hash_update_x4_4(hash_context_x4* ctx, const u8* data0,
                                    const u8* data1, const u8* data2,
                                    const u8* data3, size_t size) {
  const u8* data[4] = {data0, data1, data2, data3};
  hash_update_x4(ctx, data, size);
}

static inline void hash_update_x4_1(hash_context_x4* ctx, const u8* data, size_t size) {
  const u8* tmp[4] = {data, data, data, data};
...
static inline void hash_squeeze_x4_4(hash_context_x4* ctx, u8* buffer0, u8* buffer1,
                                     u8* buffer2, u8* buffer3, size_t buflen) {
  u8* buffer[4] = {buffer0, buffer1, buffer2, buffer3};
...
static inline void hash_update_x4_uint32_le(hash_context_x4* ctx, u32 data) {
  const u32 data_le = htole32(data);
  hash_update_x4_1(ctx, (const u8*)&data_le, sizeof(data_le));
}

static inline void hash_update_x4_uint32s_le(hash_context_x4* ctx, const u32 data[4]) {
  const u32 data0_le = htole16(data[0]);
  const u32 data1_le = htole16(data[1]);
  const u32 data2_le = htole16(data[2]);
  const u32 data3_le = htole16(data[3]);
  hash_update_x4_4(ctx, (const u8*)&data0_le, (const u8*)&data1_le,
                   (const u8*)&data2_le, (const u8*)&data3_le, sizeof(data[0]));
}
