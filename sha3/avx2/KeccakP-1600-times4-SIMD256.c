...
#define laneIndex(instanceIndex, lanePosition) ((lanePosition)*4 + instanceIndex)
    ...
    #define LOAD4_64(a, b, c, d)    _mm256_set_epi64x((u64)(a), (u64)(b), (u64)(c), (u64)(d))
    #define ROL64in256(d, a, o)     d = _mm256_or_si256(_mm256_slli_epi64(a, o), _mm256_srli_epi64(a, 64-(o)))
    #define ROL64in256_8(d, a)      d = _mm256_shuffle_epi8(a, CONST256(rho8))
    #define ROL64in256_56(d, a)     d = _mm256_shuffle_epi8(a, CONST256(rho56))
static const u64 rho8[4] = {0x0605040302010007, 0x0E0D0C0B0A09080F, 0x1615141312111017, 0x1E1D1C1B1A19181F};
static const u64 rho56[4] = {0x0007060504030201, 0x080F0E0D0C0B0A09, 0x1017161514131211, 0x181F1E1D1C1B1A19};
...
void KeccakP1600times4_AddLanesAll(void *states, const unsigned char *data, unsigned int laneCount, unsigned int laneOffset)
{
    ...
    unsigned int i;
    const u64 *curData0 = (const u64 *)data;
    const u64 *curData1 = (const u64 *)(data+laneOffset*SnP_laneLengthInBytes);
    const u64 *curData2 = (const u64 *)(data+laneOffset*2*SnP_laneLengthInBytes);
    const u64 *curData3 = (const u64 *)(data+laneOffset*3*SnP_laneLengthInBytes);
...
void KeccakP1600times4_ExtractBytes(const void *states, unsigned int instanceIndex, unsigned char *data, unsigned int offset, unsigned int length)
{
    ...
    const u64 *statesAsLanes = (const u64 *)states;
    ...
    while(sizeLeft >= SnP_laneLengthInBytes) {
        *(u64*)curData = statesAsLanes[laneIndex(instanceIndex, lanePosition)];
        sizeLeft -= SnP_laneLengthInBytes;
        lanePosition++;
        curData += SnP_laneLengthInBytes;
    }
...
{
    u64 *curData0 = (u64 *)data;
    u64 *curData1 = (u64 *)(data+laneOffset*1*SnP_laneLengthInBytes);
    u64 *curData2 = (u64 *)(data+laneOffset*2*SnP_laneLengthInBytes);
    u64 *curData3 = (u64 *)(data+laneOffset*3*SnP_laneLengthInBytes);

    const V256 *stateAsLanes = (const V256 *)states;
    const u64 *stateAsLanes64 = (const u64*)states;
    V256    lanes0, lanes1, lanes2, lanes3, lanesL01, lanesL23, lanesH01, lanesH23;
    unsigned int i;
...
void KeccakP1600times4_ExtractAndAddBytes(const void *states, unsigned int instanceIndex, const unsigned char *input, unsigned char *output, unsigned int offset, unsigned int length)
{
    ...
    if ((sizeLeft > 0) && (offsetInLane != 0)) {
        unsigned int bytesInLane = SnP_laneLengthInBytes - offsetInLane;
        u64 lane = statesAsLanes[laneIndex(instanceIndex, lanePosition)] >> (8 * offsetInLane);
        if (bytesInLane > sizeLeft)
            bytesInLane = sizeLeft;
        sizeLeft -= bytesInLane;
        do {
    ...
    while(sizeLeft >= SnP_laneLengthInBytes) {
        *((u64*)curOutput) = *((u64*)curInput) ^ statesAsLanes[laneIndex(instanceIndex, lanePosition)];
        sizeLeft -= SnP_laneLengthInBytes;
        lanePosition++;
    ...
    if (sizeLeft != 0) {
        u64 lane = statesAsLanes[laneIndex(instanceIndex, lanePosition)];
        do {
            *(curOutput++) = *(curInput++) ^ (unsigned char)lane;
            lane >>= 8;
        } while ( --sizeLeft != 0);
...
{
    const u64 *curInput0 = (u64 *)input;
    const u64 *curInput1 = (u64 *)(input+laneOffset*1*SnP_laneLengthInBytes);
    const u64 *curInput2 = (u64 *)(input+laneOffset*2*SnP_laneLengthInBytes);
    const u64 *curInput3 = (u64 *)(input+laneOffset*3*SnP_laneLengthInBytes);
    u64 *curOutput0 = (u64 *)output;
    u64 *curOutput1 = (u64 *)(output+laneOffset*1*SnP_laneLengthInBytes);
    u64 *curOutput2 = (u64 *)(output+laneOffset*2*SnP_laneLengthInBytes);
    u64 *curOutput3 = (u64 *)(output+laneOffset*3*SnP_laneLengthInBytes);
...
static ALIGN(KeccakP1600times4_statesAlignment) const u64 KeccakF1600RoundConstants[24] = {
...
size_t KeccakF1600times4_FastLoop_Absorb(void *states, unsigned int laneCount, unsigned int laneOffsetParallel, unsigned int laneOffsetSerial, const unsigned char *data, size_t dataByteLen)
{
    if (laneCount == 21) {
        ...
        const u64 *curData0 = (const u64 *)data;
        const u64 *curData1 = (const u64 *)(data+laneOffsetParallel*1*SnP_laneLengthInBytes);
        const u64 *curData2 = (const u64 *)(data+laneOffsetParallel*2*SnP_laneLengthInBytes);
        const u64 *curData3 = (const u64 *)(data+laneOffsetParallel*3*SnP_laneLengthInBytes);
...
#define _mm256_extract_epi64(a, index) \
    ((u64)_mm256_extract_epi32((a), (index)*2) || ((u64)_mm256_extract_epi32((a), (index)*2+1) << 32))
#endif
...
ATTRIBUTE_TARGET_AVX2
size_t KeccakP1600times4_KravatteCompress(u64 *xAccu, u64 *kRoll, const unsigned char *input, size_t inputByteLen)
{
    u64 *in64 = (u64 *)input;
...
ATTRIBUTE_TARGET_AVX2
size_t KeccakP1600times4_KravatteExpand(u64 *yAccu, const u64 *kRoll, unsigned char *output, size_t outputByteLen)
{
