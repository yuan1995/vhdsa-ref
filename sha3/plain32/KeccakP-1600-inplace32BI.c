...
#define ROL32(a, offset) ((((u32)a) << (offset)) ^ (((u32)a) >> (32-(offset))))
...
void KeccakP1600_SetBytesInLaneToZero(void *state, unsigned int lanePosition, unsigned int offset, unsigned int length)
{
    ...
    u32 *stateAsHalfLanes = (u32*)state;
    ...
    low = *((u32*)(laneAsBytes+0));
    high = *((u32*)(laneAsBytes+4));
#else
    low = laneAsBytes[0]
        | ((u32)(laneAsBytes[1]) << 8)
        | ((u32)(laneAsBytes[2]) << 16)
        | ((u32)(laneAsBytes[3]) << 24);
    high = laneAsBytes[4]
        | ((u32)(laneAsBytes[5]) << 8)
        | ((u32)(laneAsBytes[6]) << 16)
        | ((u32)(laneAsBytes[7]) << 24);
#endif
...
void KeccakP1600_AddByte(void *state, unsigned char byte, unsigned int offset)
{
    ...
    if (offsetInLane < 4) {
        low = (u32)byte << (offsetInLane*8);
        high = 0;
    }
    else {
        low = 0;
        high = (u32)byte << ((offsetInLane-4)*8);
    }
...
void KeccakP1600_AddLanes(void *state, const unsigned char *data, unsigned int laneCount)
{
    ...
    for(lanePosition=0; lanePosition<laneCount; lanePosition++) {
        ...
        u32 low = laneAsBytes[0]
            | ((u32)(laneAsBytes[1]) << 8)
            | ((u32)(laneAsBytes[2]) << 16)
            | ((u32)(laneAsBytes[3]) << 24);
        u32 high = laneAsBytes[4]
            | ((u32)(laneAsBytes[5]) << 8)
            | ((u32)(laneAsBytes[6]) << 16)
            | ((u32)(laneAsBytes[7]) << 24);
        u32 even, odd, temp, temp0, temp1;
        u32 *stateAsHalfLanes = (u32*)state;
        toBitInterleavingAndXOR(low, high, stateAsHalfLanes[lanePosition*2+0], stateAsHalfLanes[lanePosition*2+1], temp, temp0, temp1);
    }
#endif
...
void KeccakP1600_ExtractAndAddLanes(const void *state, const unsigned char *input, unsigned char *output, unsigned int laneCount)
{
    ...
    for(lanePosition=0; lanePosition<laneCount; lanePosition++) {
        ...
        laneAsBytes[6] = (high >> 16) & 0xFF;
        laneAsBytes[7] = (high >> 24) & 0xFF;
        ((u32*)(output+lanePosition*8))[0] = ((u32*)(input+lanePosition*8))[0] ^ (*(const u32*)(laneAsBytes+0));
        ((u32*)(output+lanePosition*8))[1] = ((u32*)(input+lanePosition*8))[0] ^ (*(const u32*)(laneAsBytes+4));
    }
#endif
}
/* ---------------------------------------------------------------- */
...
static const u32 KeccakF1600RoundConstants_int2[2*24+1] =
{
...
void KeccakP1600_Permute_Nrounds(void *state, unsigned int nRounds)
{
    ...
    u32 Ba, Be, Bi, Bo, Bu;
    u32 Cx, Cy, Cz, Cw;
    const u32 *pRoundConstants = KeccakF1600RoundConstants_int2+(24-nRounds)*2;
    u32 *stateAsHalfLanes = (u32*)state;
