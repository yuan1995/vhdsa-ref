...
typedef struct {
  u8 ctx[200];        /* param block */
  u8 data_block[168]; /* buffer for input and output data */
  u8 func;            /* function: SHAKE128 or SHKAE256 */
  u8 data_block_size; /* block size */
  u8 pos;             /* current position in data_block */
 ...
 */
static inline void hash_update_kimd(hash_context* ctx, const u8* data, size_t size) {
  /* function code in GR 0 */
  register long func asm("0") = ctx->func;
  /* param block in GR 1 */
  register u8* param asm("1") = ctx->ctx;
  /* input data in an even numbered GR (goes into R2) */
  register const u8* src asm("2") = data;
 ...
 */
static inline void hash_update_klmd(hash_context* ctx, u8* buffer, size_t buffer_size,
 ...
 */
static inline void hash_squeeze_kmld(hash_context* ctx, u8* buffer, size_t size) {
