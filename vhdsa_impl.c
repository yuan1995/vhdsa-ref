...
ATTR_PURE static inline u8* signature_c(u8* base_ptr, unsigned int index,
...
ATTR_PURE static inline u8* signature_u_tilde(u8* base_ptr,
                                                   const vhdsa_paramset_t* params) {
  const unsigned int lambda_bytes  = params->lambda / 8;
  const unsigned int ell_bytes     = params->l / 8;
  const unsigned int ell_hat_bytes = ell_bytes + 3 * lambda_bytes + UNIVERSAL_HASH_B;
...
ATTR_PURE static inline u8* signature_d(u8* base_ptr, const vhdsa_paramset_t* params) {
  const unsigned int lambda_bytes  = params->lambda / 8;
  const unsigned int ell_bytes     = params->l / 8;
  const unsigned int ell_hat_bytes = ell_bytes + 3 * lambda_bytes + UNIVERSAL_HASH_B;
...
ATTR_PURE static inline u8* signature_a1_tilde(u8* base_ptr,
                                                    const vhdsa_paramset_t* params) {
  const unsigned int lambda_bytes  = params->lambda / 8;
  const unsigned int ell_bytes     = params->l / 8;
  const unsigned int ell_hat_bytes = ell_bytes + 3 * lambda_bytes + UNIVERSAL_HASH_B;
...
ATTR_PURE static inline u8* signature_a2_tilde(u8* base_ptr,
                                                    const vhdsa_paramset_t* params) {
  const unsigned int lambda_bytes  = params->lambda / 8;
  const unsigned int ell_bytes     = params->l / 8;
  const unsigned int ell_hat_bytes = ell_bytes + 3 * lambda_bytes + UNIVERSAL_HASH_B;
...
ATTR_PURE static inline u8* signature_decom_i(u8* base_ptr,
                                                   const vhdsa_paramset_t* params) {
  const unsigned int lambda_bytes  = params->lambda / 8;
  const unsigned int ell_bytes     = params->l / 8;
  const unsigned int ell_hat_bytes = ell_bytes + 3 * lambda_bytes + UNIVERSAL_HASH_B;
...
ATTR_PURE static inline u8* signature_chall_3(u8* base_ptr,
                                                   const vhdsa_paramset_t* params) {
  const unsigned int lambda_bytes = params->lambda / 8;
  return base_ptr + params->sig_size - sizeof(u32) - IV_SIZE - lambda_bytes;
}

ATTR_PURE static inline u8* signature_iv_pre(u8* base_ptr,
                                                  const vhdsa_paramset_t* params) {
  return base_ptr + params->sig_size - sizeof(u32) - IV_SIZE;
}

ATTR_PURE static inline u8* signature_ctr(u8* base_ptr, const vhdsa_paramset_t* params) {
  return base_ptr + params->sig_size - sizeof(u32);
}

// helpers to compute position in signature (verify)

ATTR_PURE static inline const u8* dsignature_c(const u8* base_ptr, unsigned int index,
                                                    const vhdsa_paramset_t* params) {
  const unsigned int lambda_bytes  = params->lambda / 8;
  const unsigned int ell_bytes     = params->l / 8;
  const unsigned int ell_hat_bytes = ell_bytes + 3 * lambda_bytes + UNIVERSAL_HASH_B;
...
ATTR_PURE static inline const u8* dsignature_u_tilde(const u8* base_ptr,
                                                          const vhdsa_paramset_t* params) {
  const unsigned int lambda_bytes  = params->lambda / 8;
  const unsigned int ell_bytes     = params->l / 8;
  const unsigned int ell_hat_bytes = ell_bytes + 3 * lambda_bytes + UNIVERSAL_HASH_B;
...
ATTR_PURE static inline const u8* dsignature_d(const u8* base_ptr,
                                                    const vhdsa_paramset_t* params) {
  const unsigned int lambda_bytes  = params->lambda / 8;
  const unsigned int ell_bytes     = params->l / 8;
  const unsigned int ell_hat_bytes = ell_bytes + 3 * lambda_bytes + UNIVERSAL_HASH_B;
...
ATTR_PURE static inline const u8* dsignature_a1_tilde(const u8* base_ptr,
                                                           const vhdsa_paramset_t* params) {
  const unsigned int lambda_bytes  = params->lambda / 8;
  const unsigned int ell_bytes     = params->l / 8;
  const unsigned int ell_hat_bytes = ell_bytes + 3 * lambda_bytes + UNIVERSAL_HASH_B;
...
ATTR_PURE static inline const u8* dsignature_a2_tilde(const u8* base_ptr,
                                                           const vhdsa_paramset_t* params) {
  const unsigned int lambda_bytes  = params->lambda / 8;
  const unsigned int ell_bytes     = params->l / 8;
  const unsigned int ell_hat_bytes = ell_bytes + 3 * lambda_bytes + UNIVERSAL_HASH_B;
...
ATTR_PURE static inline const u8* dsignature_decom_i(const u8* base_ptr,
                                                          const vhdsa_paramset_t* params) {
  const unsigned int lambda_bytes  = params->lambda / 8;
  const unsigned int ell_bytes     = params->l / 8;
  const unsigned int ell_hat_bytes = ell_bytes + 3 * lambda_bytes + UNIVERSAL_HASH_B;
...
ATTR_PURE static inline const u8* dsignature_chall_3(const u8* base_ptr,
                                                          const vhdsa_paramset_t* params) {
  const unsigned int lambda_bytes = params->lambda / 8;
  return base_ptr + params->sig_size - sizeof(u32) - IV_SIZE - lambda_bytes;
}

ATTR_PURE static inline const u8* dsignature_iv_pre(const u8* base_ptr,
                                                         const vhdsa_paramset_t* params) {
  return base_ptr + params->sig_size - sizeof(u32) - IV_SIZE;
}

ATTR_PURE static inline const u8* dsignature_ctr(const u8* base_ptr,
                                                      const vhdsa_paramset_t* params) {
  return base_ptr + params->sig_size - sizeof(u32);
}

// VHDSA.Sign: line 3
static void hash_mu(u8* mu, const u8* owf_input, size_t owf_input_size,
                    const u8* owf_output, size_t owf_output_size, const u8* msg,
...
static void hash_iv(u8* iv, const u8* iv_pre, unsigned int lambda) {
  H4_context_t h4_ctx;
...
// VHDSA.Sign: line 4 + line 5
static void hash_r_iv(u8* root_key, u8* iv_pre, u8* iv, const u8* owf_key,
                      const u8* mu, const u8* rho, size_t rho_size, unsigned int lambda) {
...
static void hash_challenge_1(u8* chall_1, const u8* mu, const u8* hcom,
                             const u8* c, const u8* iv, unsigned int lambda,
...
static void hash_challenge_2_init(H2_context_t* h2_ctx, const u8* chall_1,
                                  const u8* u_tilde, unsigned int lambda) {
  const unsigned int lambda_bytes  = lambda / 8;
...
static void hash_challenge_2_update_v_tilde(H2_context_t* h2_ctx, const u8* v_tilde,
                                            unsigned int lambda) {
  const unsigned int lambda_bytes  = lambda / 8;
...
static void hash_challenge_2_finalize(u8* chall_2, H2_context_t* h2_ctx, const u8* d,
...
static void hash_challenge_3_init(H2_context_t* h2_ctx, const u8* chall_2,
                                  const u8* a0_tilde, const u8* a1_tilde,
                                  const u8* a2_tilde, unsigned int lambda) {
  const unsigned int lambda_bytes = lambda / 8;
...
static void hash_challenge_3_final(u8* chall_3, const H2_context_t* ctx, u32 ctr,
                                   unsigned int lambda) {
  const unsigned int lambda_bytes = lambda / 8;
...
static void hash_challenge_3(u8* chall_3, const u8* chall_2, const u8* a0_tilde,
                             const u8* a1_tilde, const u8* a2_tilde, const u8* ctr,
                             unsigned int lambda) {
  H2_context_t h2_ctx;
  hash_challenge_3_init(&h2_ctx, chall_2, a0_tilde, a1_tilde, a2_tilde, lambda);
  H2_update(&h2_ctx, ctr, sizeof(u32));
  H2_3_final(&h2_ctx, chall_3, lambda / 8);
}

static bool check_challenge_3(const u8* chall_3, unsigned int start, unsigned int lambda) {
...
static void free_pointer_array(u8*** ptr) {
  free((*ptr)[0]);
  free(*ptr);
  *ptr = NULL;
}
...
static inline void aes_prove(u8* a0_tilde, u8* a1_tilde, u8* a2_tilde,
                             const u8* w, const u8* u, u8** V, const u8* owf_in,
                             const u8* owf_out, const u8* chall_2,
                             const vhdsa_paramset_t* params) {
...
static inline void aes_verify(u8* a0_tilde, const u8* d, u8** Q,
                              const u8* chall_2, const u8* chall_3,
                              const u8* a1_tilde, const u8* a2_tilde,
                              const u8* owf_in, const u8* owf_out,
                              const vhdsa_paramset_t* params) {
  switch (params->lambda) {
  case 256:
...
// VHDSA.Sign()
void vhdsa_sign(u8* sig, const u8* msg, size_t msg_len, const u8* owf_key,
  ...
  // ::8
  u8 chall_1[(5 * MAX_LAMBDA_BYTES) + 8];
  ...
  {
    u8 V_tilde[MAX_LAMBDA_BYTES + UNIVERSAL_HASH_B];
...
int vhdsa_verify(const u8* msg, size_t msglen, const u8* sig, const u8* owf_input,
  ...
  {
    const u8* chall_3 = dsignature_chall_3(sig, params);
    u8 Q_tilde[MAX_LAMBDA_BYTES + UNIVERSAL_HASH_B];
    for (unsigned int i = 0; i != lambda; ++i) {
      // Step 12
  ...
  // Step 18
  const u8* d = dsignature_d(sig, params);
  u8 a0_tilde[MAX_LAMBDA_BYTES];
